...
<div class="container-fluid" id="wrapper">
				...
				<div class="sear-wrap ">
					...
					<div class="md3">
						<select class="form-control search-select" name="condition">
							<option value="notice_title" ${pc.paging.condition == 'notice_title' ? 'selected' : '' }>제목</option>
							<option value="notice_content" ${pc.paging.condition == 'notice_content' ? 'selected' : '' }>내용</option>
							<option value="notice_writer" ${pc.paging.condition == 'notice_writer' ? 'selected' : ''}>작성자</option>
							<option value="notice_type" ${pc.paging.condition == 'notice_type' ? 'selected' : '' }>분류</option>
						</select>
					</div>
 ...
 --%>
							...
							<td><a href="<c:url value='/noticeboard/view/${vo.notice_id}${pc.makeURI(pc.paging.pageNum)}' />"> ${vo.notice_title}</a></td>
			...
			<form action="<c:url value='/noticeboard/list' />" name="pageForm">
				<div class="text-center clearfix">
					...
					<c:if test="${login.type==1 }">
						...
						<button type="button" class="btn btn-info" onclick="location.href='<c:url value="/noticeboard/write" />'">글 등록</button>
					</c:if>
				</div>
