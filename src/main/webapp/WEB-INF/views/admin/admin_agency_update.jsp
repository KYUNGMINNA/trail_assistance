...
<div class="container-fluid" id="wrapper">
	<div class="row">
		...
		<div class="col-xs-8 col-md-8 col-lg-8 content-wrap">
			...
			<form action="<c:url value='/admin/comUpdate' />" name="comUpdateForm" id="comUpdateForm" method="post">
				<table class="table">
					<tbody class="t-control">
						<tr>
							<td class="t-title">기관번호</td>
							<td><input class="form-control input-sm" name="comNo" id="comNo" readonly="readonly" value="${comDetail.comNo}"></td>
						</tr>
						<tr>
							<td class="t-title">기관명</td>
							<td><input class="form-control input-sm" id="comName" name="comName" value="${comDetail.comName}">
								<div class="text-right">
									<span id="msgName">*중복확인을 진행하세요</span>
									<button type="button" class="btn btn-primary" id="nameCheckBtn">이름중복체크</button>
								</div> <!--자바스크립트에서 추가--></td>
						</tr>
						<tr>
							<td class="t-title">전화번호</td>
							<td><input class="form-control input-sm" name="comPhone" value="${comDetail.comPhone}"></td>
						</tr>
						<tr>
							<td class="t-title">주소</td>
							<td><input class="form-control input-sm" type="text" id="comAddress" name="comAddress" value="${comDetail.comAddress}">
								<div class="text-right">
									<input type="button" class="btn btn-primary" onclick="kakaoMap()" value="주소 검색"> <br>
								</div>
								<div id="map" style="width: 100%; height: 300px; margin-top: 10px;"></div></td>
						</tr>
					</tbody>
				</table>
			...
			<div class="com-update-btn-wrap text-right">
				<button type="button" class="btn btn-primary" id="com-modify-update-btn">수정</button>
				<button type="button" class="btn btn-dark" id="comListBtn" onclick="location.href='${pageContext.request.contextPath}/admin/admin_agency_list?pageNum=${p.pageNum}&cpp=${p.cpp}&condition=${p.condition}&keyword=${p.keyword}'">목록</button>
				<button type="button" class="btn btn-danger" id="com-update-del-btn">삭제</button>
			</div>
		</div>
	</div>
...
<script src="//dapi.kakao.com/v2/maps/sdk.js?appkey=f27612d7a2e107af3f92b69aff8eb8af&libraries=services"></script>
...
<script>
	...
	$(function() {
		...
		//수정 버튼 처리
		$('#com-modify-update-btn').click(function() {
			if ($('input[name=comName]').val() === '') {
				...
				console.log($('#comName').attr('readonly'));
			}
		});
		...
		//삭제 버튼 처리
		$('#com-update-del-btn').click(function() {
