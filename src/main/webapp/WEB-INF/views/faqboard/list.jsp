				...
				<div class="sear-wrap ">
					...
					<div class="md3">
						<select class="form-control search-select" name="condition">
							<option value="faq_title" ${pc.paging.condition == 'faq_title' ? 'selected' : '' }>제목</option>
							<option value="faq_content" ${pc.paging.condition == 'faq_content' ? 'selected' : '' }>내용</option>
							<option value="faq_type" ${pc.paging.condition == 'faq_type' ? 'selected' : '' }>FAQ 유형</option>
						</select>
					</div>
				...
				<tbody style="text-align: center;">
					<c:forEach var="vo" items="${faqList}">
						<tr>
							...
							<td>${vo.faq_type}</td>
							<td><a href="<c:url value='/faqboard/view/${vo.faq_id}${pc.makeURI(pc.paging.pageNum)}' />"> ${vo.faq_title}</a> &nbsp;&nbsp;&nbsp;</td>

							<c:if test="${login.type == 1}">
								<td><fmt:formatDate value="${vo.faq_regdate}" pattern="yyyy-MM-dd HH:mm" /></td>
			...
			<form action="<c:url value='/faqboard/list' />" name="pageForm">
				<div class="text-center clearfix">
					...
					<c:if test="${login.type==1 }">
						...
						<button type="button" class="btn btn-info" onclick="location.href='<c:url value="/faqboard/write" />'">글 등록</button>
					</c:if>
				</div>
